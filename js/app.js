    ...
    for (let i = 0; i < filesInput.files.length; i++) {
      if (
        !["image/jpeg", "image/png", "image/gif"].includes(
          filesInput.files[i].type
        )
      ) {
        return alert("Разрешены только изображения!");
      }
      const reader = new FileReader();
      reader.onload = function (e) {
        filesBody.innerHTML += `
      <li>
      <img src='${e.target.result}' alt='photo${i}' />
      </li>
      `;
      };
...
let listMenu = document.querySelector(".header__menu-list");
if (iconMenu != null) {
  let delay = 500;
  ...
  body.addEventListener("click", (e) => {
    if (
      iconMenu.classList.value.includes("_active") &&
      !e.target.classList.value.includes("header__menu-list") &&
      !e.target.closest(".header__menu-list")
    ) {
      if (unlock) {
        body_lock(delay);
        menu_close();
...
function form_submit(e) {
  ...
  let message = form.getAttribute("data-message") || "hey";
